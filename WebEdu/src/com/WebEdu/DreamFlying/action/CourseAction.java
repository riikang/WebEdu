...
public class CourseAction extends ActionSupport {
	...
	@Resource private Take_classServiceDao take_classServiceDao;
	private List<Course> courses;
	private List<Test> tests;
	private List<Take_class> tcs;
	private List<Resources> resources;
	private List<Resources> res_video;
	private List<Resources> res_document;
	private List<Comment> comments;
	private List<Student> students;
	private Take_class take_class=new Take_class();
	...
	public Take_class getTake_class() {
		return take_class;
	}
	public void setTake_class(Take_class take_class) {
		this.take_class = take_class;
	}
	public List<Take_class> getTcs() {
		return tcs;
	}
	public void setTcs(List<Take_class> tcs) {
		this.tcs = tcs;
	}
	...
	public String findTheCourse() throws Exception {
		...
		if(user_name.equals("登录")||user_name==null){
			return "getCourse_fail";
		}else{
		//查询数据
	...
	public String findMyCourse(){
		if(ActionContext.getContext().getApplication().get("username")==null||
				ActionContext.getContext().getApplication().get("username").equals("登录")){
				return "findMyCourse_fail";
		}else{
			String user_name=String.valueOf(ActionContext.getContext().getApplication().get("username"));
			students=studentService.findByName(user_name);
			tcs=take_classServiceDao.findTake_classBysid(students.get(0).getSid());
			System.out.println(tcs.size());
			return "findMyCourse_success";
		}
	}
	...
	public String updateHeat() throws Exception{
		...
		tcs=take_classServiceDao.findTake_classBysid(students.get(0).getSid());
		String cid2=String.valueOf(course.getCid());
		String cid="";
		boolean sum=true;
		for(int i=0;i<tcs.size();i++){
			cid=String.valueOf(tcs.get(i).getCourse_id().getCid());
			if(cid.equals(cid2)){
				sum=false;
			}
		}
		...
		if(sum){
			take_class.setStudent_id(students.get(0));
			take_class.setCourse_id(course);
			take_classServiceDao.save(take_class);
